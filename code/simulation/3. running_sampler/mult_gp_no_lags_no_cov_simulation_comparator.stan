...
model {
  ...
  // priors
  // variance (little_sigma) should use inv_gamma and precision should use gamma (phi, tau) - 7/11/24
  little_sigma2 ~ lognormal(log(0.3), 0.2); // inv_gamma(5,5); // uniform(0,2.5);
  phi ~ lognormal(log(0.3), 0.2); // inv_gamma(5,5); // uniform(0,2.5);
  tau ~ lognormal(log(0.3), 0.2); // inv_gamma(5,5); // uniform(0,2.5);
  for (coeff in 1:num_elements(beta)){
    beta[coeff] ~ std_normal(); // independent standard normal priors on each beta coefficient 
  }
} 
