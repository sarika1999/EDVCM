...
data <- readRDS(paste0(data_dir, 'rand_mult_gp_general_smooth_med_comparator', simulation, '.rds'))
Y <- readRDS(paste0(data_dir, 'rand_Y_general_smooth_med_comparator', simulation, '.rds'))
...
suppressMessages(
test <- stan(
  file = 'mult_gp_no_lags_no_cov_simulation_comparator.stan',  # Stan program
...
write.csv(posterior, 
          paste0(output_dir, 'rand_smooth_med_comparator_', sim_val, '.csv'),
          row.names=FALSE)
