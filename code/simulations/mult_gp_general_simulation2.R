# simulation to confirm that sampler recovers the true beta in general case
# do not assume same number of total cases in each strata 
# do not assume all beta are the same across d, t 
...
# assume t = 1 for all d is the largest and decreases in integer value for consecutive days (t = t+1, t+2)
beta <- c(1,2,1,3,2,1)
...
set.seed(1999)
sample_size <- sample((1:25), length(sequence), replace = TRUE)
...
saveRDS(test, '/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/multinomial_GP/output/simulations/allfloodzips_dur3_general_simulation2.rds')
